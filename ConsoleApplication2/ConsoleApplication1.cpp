...
void Clicker() {
    while (true) {
        if (running && (GetAsyncKeyState(VK_LBUTTON) & 0x8000)) {
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(1000 / cps - 10));
        }
        else {
...
void ShowMenu() {
    while (true) {
        system("cls");
        std::cout << "=== Меню кликера для Minecraft Windows 10 Edition ===" << std::endl;
        ...
        case 1:
            ...
            std::cout << "Зажмите ЛКМ для активации автоклика." << std::endl;
